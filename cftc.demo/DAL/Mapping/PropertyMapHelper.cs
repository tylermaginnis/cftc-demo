...
namespace cftc.demo.DAL.Mapping
{
    public class PropertyMapHelper
    {
        ...
        private static void ParsePrimitive(PropertyInfo prop, object entity, object value)
        {
            ...
            else if (prop.PropertyType == typeof(decimal))
            {
                ...
                {
                    prop.SetValue(entity, decimal.Parse(value.ToString().Substring(0, value.ToString().IndexOf("E"))), null);
                }
                else
                {
            ...
            else if (prop.PropertyType == typeof(DateTime) || prop.PropertyType == typeof(Nullable<DateTime>))
            {
                ...
                {
                    isValid = DateTime.TryParseExact(value.ToString(), "MMddyyyy", new System.Globalization.CultureInfo("en-US"), DateTimeStyles.AssumeLocal, out date);
                    if (isValid)
                    {
                        prop.SetValue(entity, date, null);
                    }
